...
#define CORE_DATA_MODEL_NAME @"LightningFunction"

@interface CDManagerSuperclass()
...
@implementation CDManagerSuperclass
...
- (NSPersistentStoreCoordinator *)persistentStoreCoordinator
{
    ...
    NSURL *storeURL = [[self applicationDocumentsDirectory] URLByAppendingPathComponent:@"BooklApp.sqlite"];
...
{
    return [self CDObjectFromEntityName:entity inMOC:self.masterManagedObjectContext];
}
...
{
    NSError *error = nil;
    NSArray *results = [self.masterManagedObjectContext executeFetchRequest:fetchRequest error:&error];
    if (error!=nil) {
        NSString *errorDesc = [NSString stringWithFormat:@"Error from fetch: %@",error.description];
        NSAssert(nil,errorDesc);
    }
    return results;
}
